...
sub col_wise {
    ...
    return map { $grid->[$_][$y] } $x .. $x + 3;
}

sub diag_down {
    my ($grid, $x, $y) = @_;

    return map { $grid->[$x++][$_] } $y .. $y + 3;
}

sub diag_up {
    my ($grid, $x, $y) = @_;

    return map { $grid->[$x--][$_] } $y-3 .. $y;
}

sub product {
    my $type = shift;
    say STDERR "$type <", join(", ", @_), ">";
    return reduce { $a * $b } @_;
}

# say "row(1,3) => ", join ", ", row_wise($grid, 1, 3);
# say "col(1,3) => ", join ", ", col_wise($grid, 1, 3);
# say "ddo(8,3) => ", join ", ", diag_down($grid, 8, 3);
# say "dup(8,3) => ", join ", ", diag_up($grid, 8, 3);

my @products;

for my $row (0..19) {
    for my $col (0..16) {
        push @products, product( "row($row, $col)", row_wise( $grid, $row, $col ) );
    }
}

for my $row (0..16) {
    for my $col (0..19){
        push @products, product( "col($row, $col)", col_wise( $grid, $row, $col ) );
    }
}

for my $row (0..16) {
    for my $col (0..16){
        push @products, product( "dd($row, $col)", diag_down( $grid, $row, $col ) );
    }
}

for my $row (3..19) {
    for my $col (3..19){
        push @products, product( "du($row, $col)", diag_up( $grid, $row, $col ) );
    }
}
